...
const TributeForm: React.FC<TributeFormProps> = ({ memorialId, fullName }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (author.trim() && message.trim()) {
      const tributeData: Omit<Tribute, 'id' | 'createdAt' | 'likes'> = {
          author,
          message,
      ...
      if (memorial && memorial.userId) {
          ...
          if (owner) {
              sendNewTributeNotification(memorial, tributeData, owner, apiSettings);
          }
      }
    ...
    <div className="mt-6 bg-pale-sky p-6 rounded-lg shadow-sm border border-silver">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
         ...
         <div>
          ...
          <textarea
            ...
            rows={4}
            placeholder="Share a memory or send condolences..."
            required
            className={inputStyles}
          ></textarea>
        </div>
        ...
        <div className="pt-4 border-t border-silver space-y-3">
            <h4 className="text-base font-semibold text-deep-navy/90">Need inspiration?</h4>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <input 
                    ...
                    placeholder="Your relationship (e.g., Friend)" 
                    className={`${inputStyles}`}
                    list="common_relationships"
                />
                 <datalist id="common_relationships">
                    <option value="Friend" />
                ...
                <select value={sentiment} onChange={(e) => setSentiment(e.target.value)} className={inputStyles}>
             ...
             <div className="text-center">
                <button 
                    ...
                    disabled={isLoading}
                    className="w-full sm:w-auto inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-deep-navy bg-silver hover:bg-soft-gray/60 disabled:bg-soft-gray/40"
                >
                    {isLoading ? (
                        <>
            ...
            {suggestions.length > 0 && (
                <div className="space-y-2">
                    <p className="text-sm text-soft-gray">Click a suggestion to use it:</p>
                    {suggestions.map((s, i) => (
                        <div key={i} onClick={() => handleSuggestionClick(s)} className="p-3 bg-white border border-silver rounded-md cursor-pointer hover:bg-dusty-blue/10">
                            <p className="text-deep-navy/90 italic">"{s}"</p>
                        </div>
                    ))}
                </div>
            )}
        ...
        <div className="text-right">
          <button
            type="submit"
            className="inline-flex items-center px-6 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-dusty-blue hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-dusty-blue"
          >
            Post Tribute
          </button>
        </div>
