...
const TributeForm: React.FC<TributeFormProps> = ({ memorialId, fullName, onSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (author.trim() && message.trim()) {
      ...
      const tributeData = {
        author,
        content: message, // mapped to content
      ...
      if (memorial && memorial.emailSettings.replyToEmail) {
        const ownerMock: User = {
          ...
          role: 'user' // Removed plan as it might not be in User type if specific
        };

        const fullTributeData: Omit<Tribute, 'id'> = { ...tributeData, likes: 0 };
        sendNewTributeNotification(memorial, fullTributeData as any, ownerMock, apiSettings, siteSettings.siteName);
      }
