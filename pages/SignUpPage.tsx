...
const SignUpPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // This is a mock signup. New users typically start on a free plan.
    const userToCreate = {
    ...
    const loggedInUser = login(userToCreate);

    // Send welcome email
    if (loggedInUser) {
        sendWelcomeEmail(loggedInUser, apiSettings);
    }

    // After signup, check if there's a pending memorial from a guest session
    if (location.state?.fromCreate && guestMemorialData) {
      const memorialToCreate: Omit<Memorial, 'id' | 'slug'> = {
        ...guestMemorialData,
        userId: loggedInUser.id, // Associate with the newly created user
