  ...
  const handleSignup = async (signupFn: () => Promise<any>) => {
    ...
    try {
      ...
      if (location.state?.fromCreate && guestMemorialData) {
        ...
        await addMemorial(memorialToCreate);
        clearGuestMemorial();
        navigate('/dashboard');
      } else {
        navigate('/dashboard');
    ...
    <div className="animate-fade-in bg-white p-8 sm:p-12 rounded-lg shadow-sm border border-silver max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input type="text" id="name" value={name} onChange={(e) => setName(e.target.value)} required className={inputStyles} autoComplete="name" disabled={isLoading} />
        </div>
        <div>
          <label htmlFor="email" className={labelStyles}>Email Address</label>
          <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} required className={inputStyles} autoComplete="email" disabled={isLoading} />
        </div>
        <div>
          <label htmlFor="password" className={labelStyles}>Password</label>
          <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} required className={inputStyles} autoComplete="new-password" disabled={isLoading} />
        </div>
        <div>
          <button type="submit" disabled={isLoading} className="w-full inline-flex justify-center py-2 px-6 border border-transparent shadow-sm text-base font-medium rounded-md text-white bg-dusty-blue hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-dusty-blue disabled:bg-soft-gray">
      ...
      <div className="mt-6">
        ...
        <div className="mt-6">
          <button type="button" onClick={handleGoogleSubmit} disabled={isLoading} className="w-full inline-flex items-center justify-center py-2 px-4 border border-silver rounded-md shadow-sm bg-white text-base font-medium text-deep-navy hover:bg-pale-sky focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-dusty-blue disabled:bg-soft-gray">
